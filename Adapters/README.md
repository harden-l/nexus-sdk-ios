...
`AdMobAdProvider` listens to Google Mobile Ads paid events for app-open, interstitial, rewarded, rewarded-interstitial, banner, and native ads. Pass `revenueReporter` and `eventReporter` to forward revenue and lifecycle events into `NexusGrowthAnalyticsAd`.
