...
@main
final class NexusRemoteDemoAppDelegate: UIResponder, UIApplicationDelegate {
    var window: UIWindow?
    ...
    func application(
        ...
        window.rootViewController = UINavigationController(rootViewController: NexusRemoteDemoViewController())
...
final class NexusRemoteDemoViewController: UIViewController {
    ...
    private let loginTypeControl = UISegmentedControl(items: NexusRemoteDemoViewController.loginTypes.map(\.rawValue))
    ...
    private func buildLayout() {
        ...
        stack.addArrangedSubview(section("CoreUserSDK", views: [
            ...
            button("Consume Chat Coins", action: #selector(consumeChatCoinsTapped)),
    ...
    private func configureFields() {
        ...
        consumeRemarkField.placeholder = "chat billing"
        consumeRemarkField.text = "chat billing"
    ...
    private func initializeGrowth(productId: String) throws {
        ...
        NexusGrowthAnalyticsAd.shared.initialize(
            ...
            providers: [dataEyeProvider],
            adProvider: nil
        )
        append("GrowthAnalyticsAd initialized with base SDK + DataEye bridge. Firebase / AppsFlyer / AdMob are tested in provider demos.")
    }
    ...
    @objc private func consumeChatCoinsTapped() {
        let costText = consumeCostField.text?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        guard let cost = Double(costText), cost > 0 else {
            append("Consume chat coins failed: cost must be greater than 0")
            return
        }
        let remark = trimmedOptionalText(consumeRemarkField)
        Task {
            do {
                try ensureCoreUserInitialized()
                let result = try await NexusCoreUser.shared.consumeChatCoins(cost: cost, remark: remark)
                await MainActor.run {
                    append(
                        "Consume chat coins success uid=\(result.uid), cost=\(result.cost), " +
                        "before=\(result.beforeCoins), after=\(result.afterCoins), balance=\(result.balance)"
                    )
                }
            } catch {
                await MainActor.run { append("Consume chat coins failed: \(error.localizedDescription)") }
            }
        }
    }
    ...
    @objc private func logoutTapped() {
        Task {
            do {
                ...
                await MainActor.run {
                    append("Logout success: server deregistered, cached user details cleared, uid retained for next login")
                }
            } catch {
    ...
    private func renderNativeAd(_ nativeAd: Any, placement: AdPlacement) {
        DispatchQueue.main.async { [weak self] in
            guard let self else { return }
            self.nativeContainer.subviews.forEach { $0.removeFromSuperview() }
            let label = UILabel()
            label.text = "Native loaded: \(placement.placement). Rendering requires the AdMob provider demo. Object: \(type(of: nativeAd))"
            label.numberOfLines = 0
            label.textAlignment = .center
            label.textColor = .secondaryLabel
            self.nativeContainer.addSubview(label)
            label.translatesAutoresizingMaskIntoConstraints = false
            NSLayoutConstraint.activate([
                label.leadingAnchor.constraint(equalTo: self.nativeContainer.leadingAnchor, constant: 12),
                label.trailingAnchor.constraint(equalTo: self.nativeContainer.trailingAnchor, constant: -12),
                label.topAnchor.constraint(equalTo: self.nativeContainer.topAnchor, constant: 12),
                label.bottomAnchor.constraint(equalTo: self.nativeContainer.bottomAnchor, constant: -12)
            ])
...
private final class DemoNativeCallbacks: NativeAdCallbacks, @unchecked Sendable {
    private let onNativeAd: @Sendable (AdPlacement, Any) -> Void
    private let log: @Sendable (String) -> Void

    init(
        onNativeAd: @escaping @Sendable (AdPlacement, Any) -> Void,
